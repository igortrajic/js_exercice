...
export function mouseMovements() {
  const cordinations = document.getElementById("mouse-coordinates");
    document.addEventListener('mousemove', (event) => {
            const x = event.clientX;
      const y = event.clientY;
      cordinations.textContent = `x: ${x}, y: ${y}`
    } );
    }
const randomRGB = () => {
  const o = Math.round
...
export function changesOnInputEvents() {
  ...
  input.addEventListener('input', () => {
    ...
    labels[0].style.color = randomRGB();
    labels[1].style.color = randomRGB();
